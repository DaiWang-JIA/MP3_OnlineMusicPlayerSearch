    ...
    , ui(new Ui::MainWidget)
{
    ...
    // [新] 初始化循环播放标志位
    isLoopPlay = false;
    ui->pushButton_Loopplay->setText(u8"循环(关)"); // 设置按钮初始文本
...
void MainWidget::on_pushButton_Loopplay_clicked()
{
    ...
    {
        // [新] 更新按钮文本
        ui->pushButton_Loopplay->setText(u8"循环(开)");
        // 你也可以在这里更改图标或样式
    }
    else
    {
        // [新] 更新按钮文本
        ui->pushButton_Loopplay->setText(u8"循环(关)");
    }
}
void MainWidget::onMediaStatusChanged(QMediaPlayer::MediaStatus status)
{
    // 检查：如果循环播放已开启 并且 媒体状态是“播放完毕”
    if (isLoopPlay == true && status == QMediaPlayer::EndOfMedia)
    {
        // 让播放器重新从头播放
        player->play();
    }
...
// 显示歌词的槽函数
void MainWidget::lyricTextShow(QString lyrics) // OK
{
...
{
    // 新的API不需要album_id，也不需要dfid/mid。
    // 它只需要 cmd=playInfo 和 hash
    Q_UNUSED(album_id); // 避免编译器警告

    QString url = kugouDownloadApi + QString("cmd=playInfo&hash=%1").arg(hash);
    ...
    auto d = connect(this, &MainWidget::finish, [&](const QByteArray & data){
        JsonData = data;
        qDebug() << "Download API Response:" << JsonData.left(200); // 打印API返回数据
        loop.exit(1);
    });
    loop.exec();
    disconnect(d);

    // 解析将要播放音乐 (使用下面已重写的 musicJsonAnalysis)
    QString music = musicJsonAnalysis(JsonData);

    // --- 这是防止崩溃的关键 ---
    if (music.isEmpty())
    {
        QMessageBox::warning(this, "播放失败", "无法获取有效的播放地址，该歌曲可能需要VIP或已下架。");
        return;
    }

    // (*** 新增 ***) 酷狗返回的URL可能是http，但实际源是https，手动替换
    if (music.startsWith("http://")) {
        music.replace(0, 4, "https");
    }

    qDebug() << "Final Play URL:" << music;

    player->setMedia(QUrl(music));

    // 设置音量 (使用滑块的当前值，而不是写死100)
    player->setVolume(ui->VopSlider->value());
...
void MainWidget::httpAccess(QString url) // OK
{
    ...
    // 实例化网络请求操作事项
        ...
        // (*** 新增 ***) 模拟浏览器 User-Agent，非常重要！
        request->setHeader(QNetworkRequest::UserAgentHeader, "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.36");
...
QString MainWidget::musicJsonAnalysis(QByteArray JsonData) // OK
{
    ...
    if ( document.isObject())
    {
        ...
        {
            QString play_url = data.value("url").toString();

            // 这个新API不返回歌词，所以我们发一个提示
            emit lyricShow("（该API暂不提供歌词）");
        ...
        {
             qDebug() << "API Error:" << data.value("error").toString();
        }
    }

    qDebug() << "musicJsonAnalysis Failed: 'url' field not found in JSON.";
    return QString(); // *** 必须有返回值，防止崩溃 ***
}
